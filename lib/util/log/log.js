...
var configure = function(app){
	...
	for (var i = 0; i < appenders.length; i++) {
		...
		if(appenders[i].type === 'file') {
			if (!!appenders[i].serverType && appenders[i].serverType !== app.serverType){
				//if server type exist but not equal with current server type then ignore it
				continue;
			}
			
			//change the log filename
			appender.filename = appender.filename.replace('[serverId]', app.serverId);
		}
		finalAppenders.push(appender);
	}
	//console.log('fileAppends: ' + JSON.stringify(finalAppenders));
