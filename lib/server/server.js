...
pro.handle = function(msg, session, cb) {
  ...
  var handle = function(err, resp) {
    if(err) {
      ...
      self.handleError(err, msg, session, resp, function(err) {
        self.afterFilter(err, msg, session, resp, cb);
      });
      return;
    }

    self.app.components.handler.handle(msg, session, function(err, resp) {
      if(err) {
        //error from handler
  ...
  this.beforeFilter(msg, session, handle);
};
...
pro.beforeFilter = function(msg, session, cb) {
  var fm = this.app.components.filter;
  if(fm) {
 ...
 * @api private
 */
pro.afterFilter = function(err, msg, session, resp, cb) {
  var fm = this.app.components.filter;
  if(fm) {
    fm.afterFilter(err, msg, session, resp, cb);
  } else {
    cb(err, resp);
  }
};
