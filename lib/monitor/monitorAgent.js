...
var STATUS_INTERVAL = 5 * 1000; // 5 seconds
...
MonitorAgent.prototype = {
    ...
    connect:function () {
        ...
        // Stop node
        monitorAgent.socket.on('stop_node', function () {
            monitorAgent._log.info("Close the node " + monitorAgent._conf.node.id);
            monitorAgent.socket.emit('stop_back', monitorAgent._conf.node);
            process.exit(0);
        });
