...
var ServerAgent = function() {
    ...
    setInterval(function() {
        self._log.info("Nodes: " + __(self.nodes).size() + ", " + "WebClients: " + __(self.web_clients).size());
    }, STATUS_INTERVAL);
...
ServerAgent.prototype = {
    ...
    announce_node: function(socket, message) {
        ...
        socket.on('onlineUserTotalCount', function(msg){
            self.connStatus[nodeId] = {count: msg.totalConnCount};
        });

        // monitorLog contains connector and rpc logsG
            ...
            socket.on('announce_web_client', function(message) {
                ...
                socket.on('webMessage',function(msg){
                    ...
                    if(msg.method==='getSenceInfo'){
                        sceneInfos=[];
												self.io.sockets.in('nodes').emit('monitorScene');
                    }
                    ...
                    if(msg.method==='getNodes'){
                        ...
                        socket.emit('getNodes',{serverArray:serverArray});
                    }
