...
var Server = function(app) {
	...
	console.error('master:%j', this.server);
	this.registered = {};
	this.masterConsole = new ConsoleService({
		type: 'master', 
...
pro.start = function(cb) {
	this.masterConsole.start(cb);
	var self = this;
	this.masterConsole.on('registered', function(record) {
		self.registered[record.id] = record;
		if(checkRegistered(this)) {
			logger.info('[master] all servers have started and notify after start now...');
			self.masterConsole.agent.notifyAll('__starter__');
		}
	});
};
