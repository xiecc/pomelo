...
app.defaultConfiguration = function () {
	...
	this.set('servers', app.get('dirname') + '/config/servers.json');
	this.set('master', app.get('dirname') + '/config/master.json');
	this.set('logger', app.get('dirname') + '/config/log4js.json');
...
app.processArgs = function(){
	...
	var serverId = hm['serverId'] === undefined ? this.master.id : hm['serverId'];
...
app.loadDefaultComponents = function(){
	...
	var remoteInterval = app.__remote__Interval || 30;
	var proxyInterval = app.__proxy__Interval || 30;
	var remoteCache = app.__remote__cache || true;
	var proxyCache = app.__proxy__cache || true;
	var lazyConnect = app.__lazyConnect || true;
	if (this.serverType === 'master') {
		app.load(pomelo.master);
	}
	else {
		if(app.get('proxy')){
			var opts = {router:this.__router__ , cacheMsg: proxyCache, interval: proxyInterval, lazyConnect: lazyConnect};
			var proxy = pomelo.proxy(this,opts);
			if(app.get('mailBoxFilter'))
				proxy.client.filter(pomelo.mailBoxFilter);
                        if(app.get('serialFilter'))
                                proxy.client.filter(pomelo.serialFilter);
                        if(app.get('timeFilter'))
                                proxy.client.filter(pomelo.timeFilter);
			app.load(proxy);
		}
		app.load(pomelo.remote, {cacheMsg: remoteCache, interval: remoteInterval});
...
app.afterStart = function(cb) {
	if(this.state !== STATE_STARTED) {
		utils.invokeCallback(cb,
				new Error('application is not running now.'));
		return;
	}
...
app.stop = function(force, cb) {
	...
	stopComps(this.loaded, 0, force, cb);
};
