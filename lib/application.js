...
app.load = function(name, component, opts) {
  ...
  if(name) {
    ...
    // NOTICE: the components with the same name would be cover by the later one.
    if(this.components[name]) {
      logger.warn('[pomelo application] duplicate name component found: %s', name);
    }
...
app.isFrontend = function(server) {
  server = server || app.findServer(app.serverType, app.serverId);
