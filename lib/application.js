...
app.loadDefaultComponents = function(){
	...
	var remoteInterval = app.remoteInterval || 30;
	var proxyInterval = app.proxyInterval || 30;
	var remoteCache = app.remoteCache || true;
	var proxyCache = app.proxyCache || true;
	var lazyConnect = app.lazyConnect || true;
	if (this.serverType === 'master') {
		app.load(pomelo.master);
	}
	else {
		if(app.get('proxy')){
			var opts = {router:this.get('calculator'), cacheMsg: proxyCache, interval: proxyInterval, lazyConnect: lazyConnect};
