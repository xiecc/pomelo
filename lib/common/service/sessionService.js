 ...
 * @api public
 */
Session.prototype.exportSession = function() {
	var res = {};
	for(var f in this) {
 ...
 * @api public
 */
Session.prototype.cloneSession = function() {
	var res = {};
 ...
 * @api public
 */
Session.prototype.closing = function() {
 ...
 * @api public
 */
Session.prototype.closed = function() {
	if(!!this._closed) {
		return;
	}
...
SessionService.kick = function(uid, cb) {
	...
	if(session) {
		...
		session.socket.send({route: 'onKick'});
...
SessionService.sendMessage = function(id, msg) {
	if(!socketMap[id]) {
		logger.warn('fail to send message for socket not exits');
		return false;
	}
...
SessionService.sendMessageByUid = function(uid, msg) {
	...
	if(!session) {
		logger.warn('fail to send message by uid for session not exist. uid: %j', uid);
		return false;
	}
...
SessionService.flush = function() {
	...
	for(var id in msgQueues) {
		...
		if(!session || !session.socket) {
			logger.warn('fail to send message for socket not exist.');
			delete msgQueues[id];
			continue;
		}
