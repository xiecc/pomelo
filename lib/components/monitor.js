...
var Monitor = function(app) {
	...
	this.monitorConsole.register(OnlineUser.moduleId, new OnlineUser(app));
	var Scripts = monitor.modules.scripts;
	this.monitorConsole.register(Scripts.moduleId, new Scripts(app));
	var Profiler = monitor.modules.profiler;
	this.monitorConsole.register(Profiler.moduleId, new Profiler(app));
};
