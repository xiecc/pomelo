...
var runServers = function (app, servers, except) {
    for (var serverType in servers) {
        ...
        var debugServerId = app.get("debugServerId");
        var debugPort = app.get("debugPort");
        for (var i = 0; i < typeServers.length; i++) {
            var curServer = typeServers[i];
            curServer.serverType = serverType;
            if (debugServerId === curServer.id)
                debug(app, curServer, debugPort);
            else
                run(app, curServer);
        }
...
var run = function (app, server) {
    ...
    if (server.host == '127.0.0.1' || server.host == 'localhost') {
        cmd = 'cd ' + process.cwd() + ' && node  ' + app.main + '  --env ' + app.env + '  --serverType ' + server.serverType + ' --serverId ' + server.id;
        starter.run(cmd);
    } else {
...
var debug = function (app, server, port) {
    var cmd = 'cd ' + process.cwd() + ' && node --debug=' + port + '  ' + app.main + ' --env ' + app.env + '  --serverType ' + server.serverType + '  --serverId ' + server.id;
    if (server.host == '127.0.0.1' || server.host == 'localhost') {
        cmd = 'cd ' + process.cwd() + ' && node --debug=' + port + '  ' + app.main + '  --env ' + app.env + '  --serverType ' + server.serverType + '  --serverId ' + server.id;
        starter.run(cmd);
    } else {
        starter.sshrun(cmd, server.host);
    }
