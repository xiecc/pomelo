...
Ext.onReady(function(){
	...
	var runScriptPanel = Ext.create('Ext.form.FormPanel', {
		...
		items:[{
			...
			border: false,
			anchor: '95%',
			items: [{xtype: 'label', text: 'Run Script:', columnWidth: .99}, serverCom, scriptCom]
		}, {
			xtype: 'textareafield',
			height: 150,
			id: 'scriptAreaId',
...
var run = function(){
	...
	window.parent.client.request('scripts', {command: 'run', serverId: serverId, script: scriptJs}, function(err, msg) {
		if(err) {
			alert(err);
			return;
		}
...
var get = function(filename){
	window.parent.client.request('scripts', {command: 'get', filename: filename}, function(err, msg) {
		if(err) {
			alert(err);
			return;
		}
...
var save = function(){
	...
	window.parent.client.request('scripts', {command: 'save', filename: filename, body: data}, function(err, msg) {
		if(err) {
			alert(err);
			return;
		}
